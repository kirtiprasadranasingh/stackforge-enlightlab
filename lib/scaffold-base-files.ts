...
export function mergeLockedBaseFiles(
  ...
  for (const [path, content] of Object.entries(base)) {
    ...
    if (path === '.github/workflows/deploy.yml') {
      console.log('deploy.yml checks:', { forceStubs, shouldForceLockPath: shouldForceLockPath(path), terraformOnly, isTf, skipGhaForce, skipWrongRuntimeStub, shouldForce, shouldFill, emptyExisting });
    }
