...
- **One resource, one file**: Never declare the same \`resource "TYPE" "NAME"\` **or** \`data "TYPE" "NAME"\` in two Terraform files. Common failures: duplicating \`google_project\` data in \`iam.tf\` + \`network.tf\`; duplicating \`google_sql_database\` across \`cloud_sql.tf\` + \`database.tf\`. Prefer a single \`data "google_project" "project"\` in \`main.tf\` or \`iam.tf\` only.
...
- **API enablement cycle**: \`google_project_service\` must set \`project = var.project_id\` (never \`data.google_project.*.project_id\`). Never put \`depends_on = [google_project_service…]\` on \`data.google_project\` — that creates \`Cycle: data.google_project, google_project_service\`.
