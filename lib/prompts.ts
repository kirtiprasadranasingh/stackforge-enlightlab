...
- **Health check vs image**: If the ECS task \`healthCheck\` uses \`curl\`/\`wget\`, the Dockerfile MUST install that binary (\`apk add --no-cache curl\` on Alpine, or use a distro image that includes it). Prefer a check the runtime can run (e.g. Node: \`node -e "fetch(...)"\` only if the base supports it, or omit container healthCheck and rely on ALB target-group \`/health\` when that is enough). Never assume Alpine Node includes \`curl\`.
