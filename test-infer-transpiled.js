...
exports.inferPresetsFromPrompt = inferPresetsFromPrompt;
exports.promptNamesCloud = promptNamesCloud;
function parseClientOverrides(prompt) {
    var out = {};
    var cloudMatch = prompt.match(/cloud provider(?:\s*\(client override\))?\s*:\s*([^.\n]+)/i);
    if (cloudMatch) {
        var c = cloudMatch[1].toLowerCase();
        if (/oracle|oci/.test(c))
            out.cloud = 'oracle';
        else if (/azure|microsoft/.test(c))
            out.cloud = 'azure';
        else if (/google|gcp/.test(c))
            out.cloud = 'gcp';
        else if (/aws|amazon/.test(c))
            out.cloud = 'aws';
    }
    ...
    if (!out.cloud) {
        if (/microsoft\s+azure[\s\S]{0,120}hosting platform(?:\s*\(client override\))?/i.test(prompt) ||
            /\bmicrosoft\s+azure\b[\s\S]{0,80}\baks\b/i.test(prompt)) {
            out.cloud = 'azure';
        }
        else if (/oracle cloud infrastructure[\s\S]{0,200}hosting platform(?:\s*\(client override\))?/i.test(prompt)) {
            out.cloud = 'oracle';
        }
        else if (/google cloud[\s\S]{0,120}hosting platform(?:\s*\(client override\))?/i.test(prompt)) {
            out.cloud = 'gcp';
        }
        else if (/\baws\b[\s\S]{0,120}hosting platform(?:\s*\(client override\))?/i.test(prompt) ||
            /amazon web services[\s\S]{0,120}hosting platform(?:\s*\(client override\))?/i.test(prompt)) {
            out.cloud = 'aws';
        }
    }
    var hostingMatch = prompt.match(/hosting platform(?:\s*\(client override\))?\s*:\s*([^.\n]+)/i);
    if (hostingMatch) {
        var h = hostingMatch[1].toLowerCase();
    ...
    var ciMatch = prompt.match(/ci\/cd system(?:\s*\(client override\))?\s*:\s*([^.\n]+)/i);
    if (ciMatch) {
        var ci = ciMatch[1].toLowerCase();
        if (/azure devops|azure pipelines/.test(ci))
            out.ci = 'azure-devops';
        else if (/gitlab/.test(ci))
            out.ci = 'gitlab-ci';
        else if (/jenkins/.test(ci))
            out.ci = 'jenkins';
        else if (/github/.test(ci))
            out.ci = 'github-actions';
        else if (/codepipeline|code pipeline|codebuild|code build/.test(ci)) {
            out.ci = 'aws-codepipeline';
        }
        else if (/cloud build|cloudbuild/.test(ci))
            out.ci = 'gcp-cloud-build';
        else if (/oci devops|oracle devops/.test(ci))
            out.ci = 'oci-devops';
    }
    if (/oracle cloud infrastructure[\s\S]{0,200}hosting platform(?:\s*\(client override\))?\s*:[^.\n]*(oke|oracle kubernetes)/i.test(prompt)) {
        out.cloud = 'oracle';
        out.orchestrator = 'oke';
    }
    if (/microsoft\s+azure[\s\S]{0,200}hosting platform(?:\s*\(client override\))?\s*:[^.\n]*(aks|azure kubernetes)/i.test(prompt)) {
        out.cloud = 'azure';
        out.orchestrator = 'aks';
    }
    return out;
}
function detectCloudFromText(rawText) {
    ...
    if (/\boke\b|oracle\s+kubernetes/.test(tCloud))
        return 'oracle';
    if (/\baks\b|azure\s+kubernetes|container\s*apps?/.test(tCloud))
        return 'azure';
    if (/\bgke\b|google\s+kubernetes|cloud\s*run/.test(tCloud))
        return 'gcp';
    if (/\beks\b|amazon\s+ecs|\becs\b|\bfargate\b/.test(tCloud))
        return 'aws';
    var mentionsOracle = /\boracle\b/.test(tCloud) ||
        /\boci\b/.test(tCloud) ||
        /\boke\b/.test(tCloud);
    var mentionsAzure = /\bazure\b/.test(tCloud) ||
        /\baks\b/.test(tCloud) ||
        /container\s*apps?/.test(tCloud) ||
        /\bazurerm\b/.test(tCloud) ||
        /microsoft\s*azure/.test(tCloud);
    var mentionsGcp = /\bgcp\b/.test(tCloud) ||
        /\bgke\b/.test(tCloud) ||
        /google\s*cloud|googlecloud/.test(tCloud) ||
        /cloud\s*run/.test(tCloud);
    var mentionsAws = /\baws\b/.test(t) ||
        /\beks\b/.test(t) ||
        /\becs\b/.test(t) ||
        /\bfargate\b/.test(t);
    if (mentionsOracle && !mentionsAws && !mentionsAzure && !mentionsGcp)
        return 'oracle';
    if (mentionsAzure && !mentionsAws && !mentionsGcp && !mentionsOracle)
        return 'azure';
    if (mentionsGcp && !mentionsAws && !mentionsAzure && !mentionsOracle)
        return 'gcp';
    if (mentionsAws && !mentionsAzure && !mentionsGcp && !mentionsOracle)
        return 'aws';
 ...
 */
function inferPresetsFromPrompt(prompt, current) {
    var _a, _b, _c, _d, _e;
    var t = prompt.toLowerCase();
    var overrides = parseClientOverrides(prompt);
    ...
    var revisionIndex = promptLines.findIndex(function (l) { return l.toLowerCase().includes('revision feedback'); });
    var latestPrompt = revisionIndex !== -1 && revisionIndex + 1 < promptLines.length
        ? promptLines.slice(revisionIndex + 1).join('\n')
        : promptLines[promptLines.length - 1] || prompt;
    var latestCloud = detectCloudFromText(latestPrompt);
    var cloud = (_c = (_b = (_a = overrides.cloud) !== null && _a !== void 0 ? _a : latestCloud) !== null && _b !== void 0 ? _b : detectCloudFromText(prompt)) !== null && _c !== void 0 ? _c : current.cloud;
    var orchestrator = (_d = overrides.orchestrator) !== null && _d !== void 0 ? _d : current.orchestrator;
    var ci = (_e = overrides.ci) !== null && _e !== void 0 ? _e : current.ci;
    var tCloud = t
        .replace(/azure\s*devops(?:\s*pipelines)?/gi, ' ')
        .replace(/azure\s*pipelines/gi, ' ')
        .replace(/google\s*cloud\s*build/gi, ' ')
        .replace(/oci\s*devops/gi, ' ');
    var mentionsAzure = cloud === 'azure';
    var mentionsAws = cloud === 'aws';
    var mentionsGcp = cloud === 'gcp';
    var mentionsOracle = cloud === 'oracle';
    var namedCloud = Boolean(cloud);
    if (!overrides.orchestrator) {
        if (namedCloud && cloud === 'azure') {
            ...
            if (/\becs\b/.test(t) || /\bfargate\b/.test(t))
                orchestrator = 'ecs';
            else
                orchestrator = 'eks';
        }
        else if (namedCloud && cloud === 'gcp') {
            if (/cloud\s*run/.test(t) || /serverless/.test(t))
...
function promptNamesCloud(prompt) {
    var t = prompt
        .toLowerCase()
        .replace(/azure\s*devops(?:\s*pipelines)?/gi, ' ')
        .replace(/azure\s*pipelines/gi, ' ');
    return (/\b(aws|azure|gcp|oci|oracle|eks|gke|aks|oke|ecs|fargate|lambda|container\s*apps?|cloud\s*run|google\s*cloud)\b/.test(t));
}
